(function() {
    ...
    const PLUGIN_PREFIX = 'fsp-floating-scene-player';
    const PLAYER_ID = `${PLUGIN_PREFIX}-container`;
    const BUTTON_ID = `${PLUGIN_PREFIX}-launch-btn`;
    const TITLE_BAR_ID = `${PLUGIN_PREFIX}-title-bar`;
    const TITLE_TEXT_ID = `${PLUGIN_PREFIX}-title-text`;
    const VIDEO_CONTAINER_ID = `${PLUGIN_PREFIX}-video-container`;
    const PLACEHOLDER_ID = `${PLUGIN_PREFIX}-placeholder`;
    const FOOTER_ID = `${PLUGIN_PREFIX}-footer`;
    const NAV_BAR_ID = `${PLUGIN_PREFIX}-nav-bar`;
    ...
    function getThemeColors(uiConfig, useTheme) {
        ...
        try {
            ...
            console.error('[Floating Scene Player] Error extracting theme colors:', error);
            return {
                background: '#1a1a1a',
                titleBar: 'linear-gradient(to bottom, #2a2a2a, #1a1a1a)',
                border: '#444',
    ...
    async function createLaunchButton(config) {
        ...
        if (launchButton || document.getElementById(BUTTON_ID)) {
            console.log('[Floating Scene Player] Button already exists, skipping creation');
            return;
        }
        ...
        `;
        launchButton.style.cssText = baseStyles + (customCSS ? '; ' + customCSS : '');
        ...
        if (document.body) {
            ...
            console.log('[Floating Scene Player] Button created and attached');
        } else {
            console.error('[Floating Scene Player] document.body not available');
        }
    }

    // Show player and update launch button
    function showPlayer() {
        console.log('[Floating Scene Player] showPlayer() called');
        if (floatingPlayer) {
    ...
    function hidePlayer() {
        console.log('[Floating Scene Player] hidePlayer() called');
        if (floatingPlayer) {
    ...
    async function createFloatingPlayer(config) {
        // Prevent duplicate creation
        if (floatingPlayer || document.getElementById(PLAYER_ID)) {
            console.log('[Floating Scene Player] Player already exists, skipping creation');
            return;
        }
        ...
        floatingPlayer.id = PLAYER_ID;

        // Base player styles - ALWAYS hidden by default
        ...
        if (document.body) {
            ...
            console.log('[Floating Scene Player] Player created and attached (hidden by default)');
        } else {
            console.error('[Floating Scene Player] document.body not available');
        }
    ...
    async function fetchSceneMetadata(sceneId) {
        try {
            ...
            console.error('[Floating Scene Player] Error fetching scene metadata:', error);
            return null;
        }
    }
    ...
    function updateNavLinks(metadata) {
        ...
        // Performer links
        if (metadata.performers && Array.isArray(metadata.performers) && metadata.performers.length > 0) {
        ...
        // Studio link
        if (metadata.studio && metadata.studio.id && metadata.studio.name) {
    ...
    async function loadVideoInPlayer(sceneId, title) {
        if (!sceneId || !floatingVideo) {
            console.warn('[Floating Scene Player] Cannot load video: missing sceneId or video element');
            return;
        }
    ...
    async function initialize() {
        console.log('[Floating Scene Player] initialize() called');
        ...
        if (settings.persistentPlayer) {
            ...
            if (!isSceneListPage) {
                console.log('[Floating Scene Player] Not on a scene list page, skipping');
                return;
            }
    ...
    // Cleanup function for non-persistent mode
    function cleanup() {
        console.log('[Floating Scene Player] cleanup() called');
    ...
    const checkNavigation = setInterval(async () => {
        if (window.location.pathname !== lastPath) {
            console.log('[Floating Scene Player] Navigation detected:', lastPath, '->', window.location.pathname);
    ...
    console.log('[Floating Scene Player] Plugin loading...');
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initialize);
    } else {
        // DOM already loaded, wait a bit for other plugins
